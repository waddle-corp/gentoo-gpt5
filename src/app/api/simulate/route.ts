...
function generateAssistantReply(score: number | undefined, summary?: string): string {
  ...
  const head = s >= 15
    ? "지금 니즈에 맞는 옵션 몇 가지를 추천드릴게요."
    : "간단히 방향부터 정리해볼게요.";
  const tail = summary ? ` (${summary})` : "";
  if (s >= 20) {
    return `${head} 예산/취향 알려주시면 더 정교하게 맞출 수 있어요.${tail}`;
  } else if (s >= 10) {
    return `${head} 검색/장바구니 이력 기준으로 무난한 선택부터 보시죠.${tail}`;
  } else {
...
function deriveEngagement(score: number | undefined) {
  ...
  const engaged = sc >= 15; // 임계값 15 이상이면 engaged
  const confidence = Math.min(1, 0.4 + sc / 40); // 0.4 ~ 1.0 근사치
...
async function findPromptsFile(): Promise<string> {
  ...
  throw new Error("simulator_prompts.json 파일을 찾을 수 없습니다. 프로젝트 루트 또는 src/data에 위치시켜주세요.");
}

export async function POST(req: Request) {
  try {
    const reqBody = await req.json().catch(() => ({}));
    const limit: number | undefined = reqBody?.limit;
    ...
    const sliced = Array.isArray(entries)
      ? (typeof limit === "number" && limit > 0 ? entries.slice(0, limit) : entries)
      : [];
    ...
    const results: ConversationResult[] = sliced.map((entry, idx) => {
      ...
      const turnDuration = 0.2 + Math.random() * 0.8; // 0.2~1.0s
    ...
    // 출력 파일 저장 (루트에 저장, 없으면 src/data로 시도)
    const cwd = process.cwd();
    const outCandidates = [
      path.join(cwd, "simulation_results.json"),
      path.join(cwd, "src", "data", "simulation_results.json"),
    ];
    ...
    if (!savedPath) {
      throw new Error("simulation_results.json 저장에 실패했습니다. 쓰기 권한을 확인하세요.");
    }
...
export async function GET(req: Request) {
  try {
    ...
    if (url.searchParams.get("prompts") === "1") {
      const promptsPath = await findPromptsFile();
      const rawPrompts = await fs.readFile(promptsPath, "utf-8");
      const entries: Array<any> = JSON.parse(rawPrompts);
      const list = entries.map((e: any, idx: number) => ({
        idx,
        user_id: e.user_id,
        engagement_score: Math.max(1, Math.min(30, Number(e?.engagement_score ?? 0) || 1)),
      }));
      return Response.json({ ok: true, count: list.length, prompts: list, path: promptsPath });
    }

    const onlyHistogram = url.searchParams.get("hist") !== "0"; // 기본: 히스토그램만

    const resultsPath = await findResultsFile();
    const raw = await fs.readFile(resultsPath, "utf-8");
    const arr: Array<any> = JSON.parse(raw);

    // engagement score 1..30 히스토그램
    ...
    return Response.json({ ok: true, count: arr.length, results: arr, histogram, path: resultsPath });
  } catch (err: any) {
    return new Response(
      JSON.stringify({ ok: false, error: err?.message || String(err) }),
      { status: 500, headers: { "content-type": "application/json" } }
